...
export default function UserPage() {
  ...
  const { userInfo, usersIsFriend, usersNotFriend, getRequest } = useSelector(
  ...
  const DeleteRequest = (_index, _userSend) => {
    ...
    dispatch(
      DeleteRequestFriend({ userGet: userData.id, userSend: _userSend })
    );
  };
      ...
      return (
        <div>
          <Button type="primary" onClick={() => {
            navigate(`/chats/detail/${parseInt(userId) + userData.id}`)
          }}>
            <span className="mr-2">Chat</span> <ChatIcon />
          </Button>
        </div>
      );
      ...
      return (
        <div>
          <Button
            ...
            type="danger"
          >
            <span className="mr-2">Add friend</span> <PersonAddIcon />
          </Button>
        </div>
      );
    }
  ...
  const contentUser = (
    <div className="relative">
      ...
      className="absolute cursor-pointer left-5 bg-black text-white bg-opacity-50 rounded-full h-10 w-10 top-16 flex justify-center items-center z-20">
        <ArrowBackIosNewIcon />
      </div>
      <div
  ...
  return (
    <div className="md:pt-10 pb-20 relative z-10">
      {contentUser}
      ...
      <div className="flex justify-center border-t  md:mt-40 mt-5">
        <div className="px-5 md:w-2/5 w-full">{contentPosts()}</div>
      </div>
    </div>
  );
}
